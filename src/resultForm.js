...
const saveResult = async (body = {}, admin) => {
  ...
  const myRequest = new Request(`https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/create/document/LGA`, {
...
function ResultForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (partyVotes1 && partyName1) {
      electionVotes.voteTotal = parseInt(electionVotes.voteTotal) + parseInt(partyVotes1);
    ...
    if (partyVotes3 && partyName3) {
      electionVotes.voteTotal = parseInt(electionVotes.voteTotal) + parseInt(partyVotes3);
      electionVotes.votesParties.push({"name": partyName3, "voteCount": partyVotes3});
    }

    if (partyVotes4 && partyName4) {
      electionVotes.voteTotal = parseInt(electionVotes.voteTotal) + parseInt(partyVotes4);
    ...
    .then(async res => {
      ...
      // const apiUrl = `https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/create/document/ElectionResults/${response.electionResultKey}`;
      // setQrValue(apiUrl); 
